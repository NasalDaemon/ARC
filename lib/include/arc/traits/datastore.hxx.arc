...
trait DataStore [Types]
{
    ...
    // Returns a pair of (pointer to SharedData, whether it was added)
    ...
    // If Event is void, no notification is sent.
    ...
    // Mutable pointers are only available when a non-void Event is returned to notify listeners with.
    // The caller is not notified; the returned Event is used to notify the other listeners after modification.
    // Returns const pointers to SharedData and PrivateData after any resulting event has been processed by the listeners.
...
trait DataListener [Types]
{
    ...
    // init: initialise PrivateData for the given id using the provided constructor.
