...
A trait is an interface presented by one node to another to satisfy a dependency. In a [cluster](cluster-syntax.md), all node dependencies are satisfied by connecting to the appropriate providing nodes by the name of the trait they implement. As a result, on each dependent node, a `my::trait::TraitName::method` on the providing node can be invoked via `getNode(my::trait::traitName).method(args...)`, and a `TypeName` can be resolved via `arc::ResolveTypes<Node, my::trait::TraitName>::TypeName`. See the example [here](modules-example.md#sessionsixx).

Traits generated by the DSL are placed inside `inline namespace trait` within their enclosing namespace. This means `my::trait::TraitName` and `my::TraitName` are both valid and refer to the same type. The `trait::` can be included or omitted at the call site. Traits are privileged in this way because they are the most commonly used entity in ARC as they provide the primary means of interaction between nodes. Placing them in an inline namespace allows for convenient access without verbose qualification.
...
```
This results in `first::second::second::third::trait::Trait`, **not** `first::second::third::trait::Trait`.
