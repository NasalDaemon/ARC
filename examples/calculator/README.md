...
- User-defined functions: `f(x, y) = x^2 + y`
- Built-in functions: `sqrt`, `sin`, `cos`, `tan`, `log`, `exp`, `abs`, etc.
...
| `Variables` | Named variable storage (get/set/list/clear) |
| `Functions` | Built-in and user-defined function registry |
...
Each node lives in `src/nodes/` with its interface in an `.ixx` file and its implementation in `impl/`. Nodes with context dependencies use the `.impl.ixx` pattern.
